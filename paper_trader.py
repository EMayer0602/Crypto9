...
def get_last_processed_timestamp() -> Optional[pd.Timestamp]:
    """Get the most recent last_processed_bar timestamp from the saved state.

    Returns the latest timestamp across all strategy keys, or None if no state exists.
    """
    state = load_state()
    ...
    latest_ts = None
    for key, ts_str in last_processed.items():
        if not ts_str:
            continue
        try:
